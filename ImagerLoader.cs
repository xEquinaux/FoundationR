...
namespace FoundationR.Rew
{
	public class RewBatch
	{
		...
		static extern void DrawRectangle(float x, float y, float width, float height);
		[DllImport(".\\Direct2D_Render.dll")]
		static extern void DrawImageBuffer(byte[] buffer, uint x, uint y, uint width, uint height);
		[DllImport(".\\Direct2D_Render.dll")]
		static extern void Initialize(IntPtr hwnd, int width, int height);
		[DllImport(".\\Direct2D_Render.dll")]
		static extern void BeginDraw();
		[DllImport(".\\Direct2D_Render.dll")]
		static extern void EndDraw();
		[DllImport(".\\Direct2D_Render.dll")]
		...
		void Init(IntPtr hwnd, int width, int height)
		{
			...
			RewBatch.width = width;
			RewBatch.height = height;
			if (Option == RenderOption.Direct2D)
				Initialize(hwnd, width, height);
			else
			if (Option == RenderOption.Both)
			{
				Initialize(hwnd, width, height);
				backBuffer = new byte[width * height * (BitsPerPixel / 8)];
			}
		...
		public bool Resize(int width, int height)
		{
			if (oldWidth != width || oldHeight != height)
			{
				...
				else if (Option == RenderOption.Both)
				{
					OnResize((uint)width, (uint)height);
					backBuffer = new byte[width * height * (BitsPerPixel / 8)];
				}
		...
		public virtual void Draw(REW image, Rectangle rectangle)
		{
			...
			if (result == null)
				return;
			if (Option == RenderOption.Direct2D)
				DrawImageBuffer(result, (uint)rectangle.X, (uint)rectangle.Y, (uint)w, (uint)h);
			else
		...
		public virtual void Draw(REW image, int x, int y)
		{
			...
			if (result == null)
				return;
			if (Option == RenderOption.Direct2D)
				DrawImageBuffer(result, (uint)x, (uint)y, (uint)w, (uint)h);
			else
		...
		public virtual void Draw(REW image, int x, int y, int width, int height)
		{
			int w = width;
			int h = height;
			byte[] result = null;
			if (Culling(width, height, x, y))
				result = CropARGBImage(image.GetPixels(), width, height, x, y, out w, out h);
			else
				result = image.GetPixels();
			if (result == null)
				return;
		...
		public virtual void Draw(byte[] image, int x, int y, int width, int height)
		{
			...
			if (image == null)
				return;
			if (Option == RenderOption.Direct2D)
				DrawImageBuffer(image, (uint)x, (uint)y, (uint)w, (uint)h);
			else
		...
		public void End(RenderOption type, IntPtr handle)
		{
			switch (type)
			{
				...
				case RenderOption.Both:
					DrawImageBuffer(backBuffer, 0, 0, (uint)Viewport.Width, (uint)Viewport.Height);
					EndDraw();
					goto default;
		...
		public virtual void CompositeImage(byte[] buffer, int bufferWidth, int bufferHeight, byte[] image, int imageWidth, int imageHeight, int x, int y, int origX, int origY, bool text = false)
		{
			...
			Parallel.For(0, imageHeight, i =>
			{
				for (int j = 0; j < imageWidth; j++)
				{
					...
					byte baseA = buffer[bufferIndex];
					byte baseR = buffer[bufferIndex + 1];
					byte baseG = buffer[bufferIndex + 2];
					byte baseB = buffer[bufferIndex + 3];

					byte overlayA = image[index];
					byte overlayR = image[index + 1];
					byte overlayG = image[index + 2];
					byte overlayB = image[index + 3];

					float alpha = overlayA / 255f;
					byte resultA = (byte)(baseA + overlayA * (1 - baseA / 255f));
					byte resultR = (byte)(baseR * (1 - alpha) + overlayR * alpha);
					byte resultG = (byte)(baseG * (1 - alpha) + overlayG * alpha);
					byte resultB = (byte)(baseB * (1 - alpha) + overlayB * alpha);

					buffer[bufferIndex] = resultA;
					buffer[bufferIndex + 1] = resultR;
					buffer[bufferIndex + 2] = resultG;
					buffer[bufferIndex + 3] = resultB;
				}
			});
		}
		public static byte[] CompositeARGB(byte[] baseArray, byte[] overlayArray)
		{
			byte[] resultArray = backBuffer;

			for (int i = 0; i < baseArray.Length; i += 4)
			{
				byte baseA = baseArray[i];
				byte baseR = baseArray[i + 1];
				byte baseG = baseArray[i + 2];
				byte baseB = baseArray[i + 3];

				byte overlayA = overlayArray[i];
				byte overlayR = overlayArray[i + 1];
				byte overlayG = overlayArray[i + 2];
				byte overlayB = overlayArray[i + 3];
				...
				resultArray[i] = resultA;
				resultArray[i + 1] = resultR;
				resultArray[i + 2] = resultG;
				resultArray[i + 3] = resultB;
			}
	...
	public partial class REW
	{
		...
		public static REW Create(int width, int height, Pixel[,] map, PixelFormat format)
		{
		...
		public static REW Dummy(int width, int height, short bpp, bool bgra)
		{
			return new REW(width, height, bpp, bgra);
		}
		public REW() { }
		...
		private REW(int width, int height, short bpp, bool bgra)
		{
			...
			WriteHeader(this);
			WriteDataChunk(this, default, bgra);
		}
		...
		static void WriteDataChunk(REW rew, Color color, bool flip)
		{
			int num = 0;
			for (int j = 0; j < rew.Height; j++)
			{
				for (int i = 0; i < rew.Width; i++)
				{
					...
					{
						pixel = new Pixel(color.A, color.R, color.G, color.B, flip);
					}
					else
					{
		...
		static void WriteDataChunk(REW rew, Pixel[,] map)
		{
			for (int j = 0; j < rew.Height; j++)
			{
				for (int i = 0; i < rew.Width; i++)
				{
					rew.SetPixel(i, j, map[i, j].color);
				}
			}
		}
		...
		public virtual void ReadData(BinaryReader br, bool argb)
		{
			Point16 size = br.ReadPoint16();
			int len = br.ReadInt32();
			BitsPerPixel = br.ReadInt16();
		...
		public virtual void SetPixel(int x, int y, Color color)
		{
			...
			if (NumChannels == 4)
			{
				data[Math.Min(data.Length - 1, whoAmI * 4 + HeaderOffset)] = color.A;      // B
				data[Math.Min(data.Length - 1, whoAmI * 4 + HeaderOffset + 1)] = color.R;  // G
				data[Math.Min(data.Length - 1, whoAmI * 4 + HeaderOffset + 2)] = color.G;  // R
				data[Math.Min(data.Length - 1, whoAmI * 4 + HeaderOffset + 3)] = color.B;  // A
			}
			else
			{
				data[Math.Min(data.Length - 1, whoAmI * 3 + HeaderOffset)] = color.R;      // B
				data[Math.Min(data.Length - 1, whoAmI * 3 + HeaderOffset + 1)] = color.G;  // G
				data[Math.Min(data.Length - 1, whoAmI * 3 + HeaderOffset + 2)] = color.B;  // R
			}
		}
	...
	public struct Pixel
	{
		...
		public byte A = 255, R, G, B;                 //B  G  R  A                  R  G  B
		...
		public static void Composite(this byte[] input, byte[] layer, int x, int y, int width, int height)
		{
			...
			Parallel.For(0, height, j =>
			{
				for (int i = 0; i < w; i++)
				{
					...
					{
						input[whoAmI] = 255;					 // B
						input[whoAmI + 1] = _two.color.R; // G
						input[whoAmI + 2] = _two.color.G; // R
						input[whoAmI + 3] = _two.color.B; // 255
					}
				}
			});
		}
