...
namespace FoundationR.Lib
{
	public partial class Foundation
	{
		...
		internal void Run(Surface window)
		{
			...
			if (RewBatch.Option != RenderOption.None)
			{
			START:
			...
			_rewBatch = new RewBatch(window.form.Handle, window.Width, window.Height, window.BitsPerPixel);
			...
			void draw(ref bool taskDone, Surface surface)
			{
				...
				while (running)
				{
					if (taskDone)
					{
						...
						{
							_rewBatch = new RewBatch(surface.form.Handle, width, height, window.BitsPerPixel);
						}
		...
		{
			_rewBatch.End(RewBatch.Option, handle);
		}
		#region events
